...
void BattleUnit::damagesAwarePathFind(std::vector<TilePosition>& btpath, 
                          const TilePosition& start, const TilePosition& end, int* tab)
{
    btpath.clear();
    MapManager* mapm = & MapManager::Instance();
    std::multimap<int, TilePosition> openTiles;
    ...
    while(!openTiles.empty())
    {
        ...
        for(int x = minx; x <= maxx; x++)
            ...
            {
                if (!mapm->lowResWalkability[x + y*width]) continue;
                if (tab[x + y*width] > 14) continue;                // damagesAware
                if (p.x() != x && p.y() != y && 
                    !mapm->lowResWalkability[p.x() + y*width] 
                    && !mapm->lowResWalkability[x + p.y()*width]
                    && tab[p.x() + y*width] > 14                    // damagesAware
                    && tab[x + p.y()*width] > 14)                   // damagesAware
                    continue;
