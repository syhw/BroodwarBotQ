...
#define _PROB_NO_FLOCK_MOVE 0.2 // TODO change for const and set specific values depending on the unit type
...
, _slarge(unit->getType().dimensionRight() + unit->getType().dimensionLeft())
{
    ...
    if (_mode == MODE_FLOCK)
    {
        ...
        _flockProb.push_back(0.2);                  //FLOCK_CONTACT
        _flockProb.push_back(0.2);                 //FLOCK_CLOSE
        _flockProb.push_back(0.2);                  //FLOCK_MEDIUM
        _flockProb.push_back(0.2);                 //FLOCK_FAR
    }
...
void BayesianUnit::computeFlockValues()
{
    ...
    for (unsigned int i = 0; i < _dirv.size(); ++i)
    {
            ...
            it != _unitsGroup->end(); ++it)
        {
            ...
            flock_value value = (flock_value)(1 + (int)tmp.getDistance(tmpvit.translate((*it)->unit->getPosition())));
        ...
        const Position& p_start, const Position& p_end, bool quick)
{
    ...
    if (quick) 
    {
        for (int id = 0; p_current != p_end && id < 2; ++id)
        {
            ...
            //Broodwar->drawLineMap(p_current.x(), 
            //        p_current.y(), p_current.x() + 
            //        _dirv[imax].x, p_current.y() + 
            //        _dirv[imax].y, Colors::Red);
...
double BayesianUnit::computeProb(unsigned int i)
{
    ...
    if (_occupation[i] == OCCUP_BUILDING)
    {
        val *= 1-_PROB_NO_BUILDING_MOVE;
    }
    else if (_occupation[i] == OCCUP_BLOCKING)
    {
        val *= 1-_PROB_NO_WALL_MOVE;
    }
...
void drawProbs(map<double, Vec>& probs, int number)
{
    Position up = unit->getPosition();
    Rainbow colors = Rainbow(Color(12, 12, 255), 51); // 5 colors
    for (map<double, Vec>::const_iterator it = probs.begin(); it != probs.end(); ++it)
    {
        if (number == 1)
        {
            const int middle = 64;
...
void BayesianUnit::updateDir()
{
    ...
    //updateProx();
    updateAttractors();
    map<double, Vec> dirvProb;
    /*for (vector<Vec>::const_iterator it = _dirv.begin(); it != _dirv.end(); ++it)
    {
        Position tmp = it->translate(p);
        dirvProb.insert(make_pair(computeProb(tmp), &(*it)));
    }*/
    for (unsigned int i = 0; i < _dirv.size(); ++i)
        dirvProb.insert(make_pair(computeProb(i), _dirv[i]));
    dir = (--dirvProb.end())->second; // that (--.end()) or 1-prob before inserting
