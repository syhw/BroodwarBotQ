...
{
	bool haveTech = true;
	const std::map< const UnitType*, int >& reqTech = unitType.requiredUnits();
	for( std::map< const UnitType*, int >::const_iterator it = reqTech.begin(); it != reqTech.end(); it++)
		if( buildManager->getCompletedCount( *(it->first)) < it->second)
			haveTech = false;
